...
def read_cams_hi4d(root):
    campath = os.path.join(root, 'cameras/rgb_cameras.npz')
    assert os.path.exists(campath), campath
    datas = np.load(campath)
    ids = datas['ids']
    intri = datas['intrinsics']
    extri = datas['extrinsics']
    dists = datas['dist_coeffs']
    cams_info = {}
    for i in range(len(ids)):
        cam_info = {
            'K': intri[i].astype(np.float32),
            'invK': np.linalg.inv(intri[i].astype(np.float32)),
            'RT': extri[i].astype(np.float32),
            'R': extri[i][:3, :3].astype(np.float32),
            'T': extri[i][:3, 3:].astype(np.float32),
            'D': dists[i].astype(np.float32)
        }
        cam_info['O'] = - cam_info['R'].T @ cam_info['T']
        cam_info['P'] = cam_info['K'] @ cam_info['RT']
        
        cams_info[str(ids[i])] = cam_info
    return cams_info
