...
class LossKeypoints3D:
    ...
    def loss(self, diff_square):
        if self.norm == 'l2':
            loss_3d = torch.sqrt((diff_square**2).sum(axis=-1)).mean(axis=-1).sum()
        elif self.norm == 'l1':
...
class LossRegPosesZero:
    def __init__(self, keypoints, cfg) -> None:
        ...
        if keypoints.shape[-2] <= 15:
            ...
            # use_feet = keypoints[..., [19, 20, 21, 22, 23, 24], -1].sum() > 0.1 # body25
            use_feet = keypoints[..., [17, 18], -1].sum() > 0.1 # skel19
            # use_head = keypoints[..., [15, 16, 17, 18], -1].sum() > 0.1 # body25
            use_head = keypoints[..., [9, 10], -1].sum() > 0.1 # skel19
        ...
        SMPL_POSES_ZERO_IDX = [[j for j in range(3*(i-1), 3*(i-1)+3)] for i in SMPL_JOINT_ZERO_IDX]
    ...
    def __call__(self, body_pose, **kwargs):
        ...
        return torch.sum(torch.abs(body_pose[:, self.idx]))/body_pose.shape[0]
...
class LossSmoothPoses:
    ...
    def _poses(self, body_pose):
        "smooth poses"
        loss = 0
        for nv in range(self.nViews):
            poses_ = body_pose[nv*self.nFrames:(nv+1)*self.nFrames, ]
            # 计算poses插值
...
class LossInit:
    def __init__(self, params, cfg) -> None:
        self.norm = 'l2'
        self.poses = torch.Tensor(params['body_pose']).to(cfg.device)
        self.shapes = torch.Tensor(params['betas']).to(cfg.device)
