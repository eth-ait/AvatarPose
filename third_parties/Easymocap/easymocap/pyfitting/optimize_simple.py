...
from easymocap.dataset.config import smpl_to_joint_model, body25_to_skel, verts_to_body25
...
def optimizeShape(body_model, body_params, keypoints3d,
    ...
    def closure(debug=False):
        optimizer.zero_grad()
        outputs = body_model(return_verts=True, return_tensor=True, only_shape=True, **body_params)
        joints = verts_to_body25(outputs.vertices)
        idxs = body25_to_skel(cfg.body)
        keypoints3d = joints[:, idxs, :]
        ...
        loss_dict = {
            ...
            'reg_shapes': torch.sum(body_params['betas']**2)}
        if 'init_shape' in weight_loss.keys():
            loss_dict['init_shape'] = torch.sum((body_params['betas'] - body_params_init['betas'])**2)
...
def get_prepare_smplx(body_params, cfg, nFrames):
    ...
    def pack(new_params):
        if not cfg.OPT_HAND and cfg.model in ['smplh', 'smplx']:
            ...
            new_params['body_pose'] = new_params['body_pose']
        return new_params
    return pack
...
def get_optParams(body_params, cfg, extra_params):
    ...
    if cfg is None:
        opt_params = [body_params['global_orient'], body_params['transl'], body_params['body_pose']]
    else:
        ...
        if cfg.OPT_SHAPE:
            opt_params.append(body_params['betas'])
...
def _optimizeSMPL(body_model, body_params, prepare_funcs, postprocess_funcs, 
    ...
    def closure(debug=False):
        ...
        out = body_model(return_verts=True, return_tensor=True, **new_params)
        joints = verts_to_body25(out.vertices)
        idxs = body25_to_skel(cfg.body)
        kpts_est = joints[:, idxs, :]
