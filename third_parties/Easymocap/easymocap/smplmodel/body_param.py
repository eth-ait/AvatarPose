...
def merge_params(param_list, share_shape=True):
    output = {}
    for key in ['body_pose', 'betas', 'global_orient', 'transl', 'expression']:
        if key in param_list[0].keys():
            output[key] = np.vstack([v[key] for v in param_list])
    if share_shape:
        output['betas'] = output['betas'].mean(axis=0, keepdims=True)
    return output
...
def select_nf(params_all, nf):
    ...
    if params_all['betas'].shape[0] == 1:
        output['betas'] = params_all['betas']
    else:
        output['betas'] = params_all['betas'][nf:nf+1, :]
    return output
...
def load_model(gender='neutral', use_cuda=True, model_type='smpl', skel_type='body25', device=None, model_path='data/SMPLX'):
    ...
    from ..smplmodel_2.body_models import SMPL
    if model_type == 'smpl':
    #     if skel_type == 'body25':
    ...
    #         raise NotImplementedError
        ...
        body_model = SMPL(join(model_path, 'smpl'), gender=gender, device=device)
