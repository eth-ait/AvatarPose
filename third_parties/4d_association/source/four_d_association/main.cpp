...
int main()
{
	std::map<std::string, Camera> cameras = ParseCameras("../../data/data2/calibration.json");
	auto camsize = cameras.size();
...
#pragma omp parallel for
	for (int i = 0; i < cameras.size(); i++) {
		...
		videos[i] = cv::VideoCapture("../../data/data2/video/" + iter->first + ".avi");
		videos[i].set(cv::CAP_PROP_POS_FRAMES, 0);

		projs.middleCols(4 * i, 4) = iter->second.eiProj;
		seqDetections[i] = ParseDetections("../../data/data2/detection/" + iter->first + ".txt");
	...
	associater.SetMaxTempDist(0.3f); //0.3f
	associater.SetMaxEpiDist(0.15f); //0.15f
	associater.SetPlaneThetaWelsh(5e-1f); //5e-3f
