...
{
	// init
	std::map<std::string, Camera> cams = ParseCameras("../../data/seq/calibration.json");
	...
	// std::vector<std::map<int, Eigen::Matrix4Xf>> gt = ParseSkels("../../data/data2/gt.txt");
	std::vector<std::map<int, Eigen::Matrix4Xf>> skels;

#pragma omp parallel for
	for (int i = 0; i < cams.size(); i++) {
		auto iter = std::next(cams.begin(), i); 
		videos[i] = cv::VideoCapture("../../data/seq/video/" + iter->first + ".avi");
		projs.middleCols(4 * i, 4) = iter->second.eiProj;
		seqDetections[i] = ParseDetections("../../data/seq/detection/" + iter->first + ".txt");
	...
	associater.SetMaxTempDist(0.3f); //0.3
	associater.SetMaxEpiDist(0.15f);
	associater.SetPlaneThetaWelsh(5e-1f); //5e-1
	associater.SetEpiWeight(1.f); //1.f
	associater.SetTempWeight(2.f);
	associater.SetViewWeight(1.f);
	associater.SetPafWeight(1.f);
	associater.SetHierWeight(0.f);//0.
	associater.SetViewCntWelsh(1.5f);
	associater.SetMinCheckCnt(2);//2
	...
	skelPainter.jointRadius = 3; //8
	skelPainter.pafThickness = 2; //10
	skelPainter.rate = 0.5f; //1.0
...
#pragma omp parallel for
	...
	SerializeSkels(skels, "../../data/seq/skel.txt");
