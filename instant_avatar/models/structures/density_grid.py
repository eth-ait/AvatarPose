...
def get_aabb(vs, scale=1.2):
    """Get axis-aligned bounding box for points (human)"""
...
class DensityGrid(torch.nn.Module):
    ...
    def update(self, deformer, net, step):
        ...
        if step < 800 and self.smpl_init:
            # initialize the grid upon the first call
            if not self.initialized:
                coords = denormalize(self.coords + 0.5 / self.grid_size, self.aabb)

                self.mesh_v_norm = deformer.vertices
                self.mesh_f_norm = deformer.body_model.faces_tensor
                self.mesh_face_vertices = index_vertices_by_faces(self.mesh_v_norm, self.mesh_f_norm)
                ...
                sign = kaolin.ops.mesh.check_sign(
                    self.mesh_v_norm, self.mesh_f_norm, coords.reshape(1, -1, 3)
    ...
    def initialize(self, deformer, net, iters=5):
        ...
        label = torch.mode(mcc[self.density_field], 0).values
        self.density_field = (mcc == label)
        # print("density sum after max component", self.density_field.sum())
